...
const lookDownHandler = async ({distance, roll, yaw, pitch, progressSetter, capture, clicked, currentActionIndex, isIndexDone, setIndexDone, wrongActionSetter} : Props) => {
  if ((roll > -20) && (roll < 20)) {
    if ((yaw > -20) && (yaw < 20)) {
...
export default lookDownHandler